...
test.describe("IE-6 markets page", () => {
  ...
  test("sections, CTAs, and responsive overflow", async ({ page }, testInfo) => {
    ...
    for (const width of [1024, 1280, 1440, 1920] as const) {
      ...
      await expect(page.locator(".hamd-aie-markets")).toBeVisible({ timeout: 30_000 });
      await expect(
      ...
      await expect(page.locator(".hamd-aie-markets__globe")).toBeVisible();
      await expect(
        page.getByRole("heading", { name: /market information is being updated/i }),
      ).toBeVisible();
      await expect(page.getByText(/\bUAE\b/)).toHaveCount(0);
      await expect(page.getByText(/50\+\s*countries/i)).toHaveCount(0);
      expect(await overflowPx(page)).toBeLessThanOrEqual(1);
    }

    await page
      .locator(".hamd-aie-markets__section--cta")
    ...
    await page.goto(MARKETS);
    await page
      .getByRole("link", { name: /see how the export process works/i })
      .first()
      .click();
    ...
    await page.goto(MARKETS);
    await page
      .locator(".hamd-aie-markets__hero")
  ...
  test("mobile markets layout has no overflow", async ({ page }, testInfo) => {
  ...
  test("axe WCAG AA on markets page", async ({ browser }, testInfo) => {
    test.skip(testInfo.project.name !== "chromium-desktop", "desktop");
    test.setTimeout(120_000);
