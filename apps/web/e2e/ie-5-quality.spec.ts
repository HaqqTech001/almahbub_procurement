...
test.describe("IE-5 quality page", () => {
  ...
  test("sections, CTAs, and responsive overflow", async ({ page }, testInfo) => {
    ...
    for (const width of [1024, 1280, 1440, 1920] as const) {
      ...
      await expect(page.locator(".hamd-aie-quality")).toBeVisible({ timeout: 30_000 });
      await expect(
      ...
      await expect(
        page.getByRole("heading", { name: /how quality relates to your requirement/i }),
      ).toBeVisible();
      await expect(page.getByRole("heading", { name: /^specification alignment$/i })).toBeVisible();
      await expect(page.getByRole("heading", { name: /^documentation$/i })).toBeVisible();
      expect(await overflowPx(page)).toBeLessThanOrEqual(1);
    }

    await page
      .locator(".hamd-aie-quality__section--cta")
    ...
    await page.goto(QUALITY);
    await page
      .locator(".hamd-aie-quality__hero")
  ...
  test("mobile quality layout has no overflow", async ({ page }, testInfo) => {
  ...
  test("axe WCAG AA on quality page", async ({ browser }, testInfo) => {
    test.skip(testInfo.project.name !== "chromium-desktop", "desktop");
    test.setTimeout(120_000);
