...
function unwrapShipmentList(payload: unknown): ApiShipment[] {
  if (Array.isArray(payload)) return payload;
  if (payload && typeof payload === "object") {
    const record = payload as Record<string, unknown>;
    if (Array.isArray(record.data)) return record.data as ApiShipment[];
    if (Array.isArray(record.items)) return record.items as ApiShipment[];
  }
  return [];
}
...
export async function listShipments(
  ...
  const rows = unwrapShipmentList(
    await shipmentFetch<unknown>("/", {
      method: "GET",
      accessToken,
      query: {
  ...
  return rows.flatMap((row) => {
    try {
      return [mapApiShipmentToRecord(row)];
    } catch {
      return [];
    }
  });
